...
export async function onRequest(context) {
  ...
  const upstream = new URL(`${POLLINATIONS_BASE}/video/${encodeURIComponent(prompt)}`);
  const allowed = ['model', 'seed', 'duration', 'aspectRatio', 'aspect-ratio', 'audio'];
  const params = pickParams(url.searchParams, allowed);
